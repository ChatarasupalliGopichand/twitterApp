...
//getting user following table

const getFollowingPeopleIdsOfUser = async (username) => {
  const getFollowingPeopleIdsOfUserQuery = `SELECT following_user_id FROM follower INNER JOIN user ON user.user_id = follower.follower_user_id
  WHERE user.username = '${username}';`;
  const followingPeople = await db.all(getFollowingPeopleIdsOfUserQuery);
...
const authenication = (request, response, next) => {
  ...
  if (jwtToken) {
    jwt.verify(jwtToken, "DDUGKY", (error, payload) => {
      if (error) {
        response.status(401);
...
app.get("/user/tweets/feed/", authenication, async (request, response) => {
  ...
  follower.following_user_id = tweet.user_id INNER JOIN user ON user.user_id = tweet.user_id INNER JOIN user ON user.user_id = follower.following_user_id
...
app.get("/user/following/", authenication, async (request, response) => {
  ...
  const getFOllowingQuery = `SELECT  name FROM follower INNER JOIN user ON user.user_id = follower.following_user_id WHERE follower_user_id = '${userId}'`;
...
app.get("/user/tweets/", authenication, async (request, response) => {
  ...
  COUNT(DISTINCT reply_id) AS replies,date_time AS dateTime FROM tweet LEFT JOIN reply ON tweet.tweet_id = reply.tweet_id 
